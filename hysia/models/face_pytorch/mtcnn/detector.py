...
from models.face_pytorch.mtcnn.models import PNet,RNet,ONet
import models.face_pytorch.mtcnn.utils as utils
import models.face_pytorch.mtcnn.image_tools as image_tools

class mtcnn_detector(object):

    def __init__(self, p_model_path=None,
            r_model_path=None,
            o_model_path=None, 
            minisize=12,
        ...
        if p_model_path is not None:
            ...
            if(use_cuda):
                ...
                self.pnet_detector.load_state_dict(torch.load(p_model_path))
                self.pnet_detector.cuda()
            else:
                # forcing all GPU tensors to be in CPU while loading
                self.pnet_detector.load_state_dict(torch.load(p_model_path, map_location=lambda storage, loc: storage))
        ...
        if r_model_path is not None:
            ...
            if (use_cuda):
                ...
                self.rnet_detector.load_state_dict(torch.load(r_model_path))
                self.rnet_detector.cuda()
            else:
                self.rnet_detector.load_state_dict(torch.load(r_model_path, map_location=lambda storage, loc: storage))
        ...
        if o_model_path is not None:
            ...
            if (use_cuda):
                ...
                self.onet_detector.load_state_dict(torch.load(o_model_path))
                self.onet_detector.cuda()
            else:
                self.onet_detector.load_state_dict(torch.load(o_model_path, map_location=lambda storage, loc: storage))
    ...
    def detect_pnet(self, im):
        ...
        current_scale = float(net_size) / self.minisize    # find initial scale
